...
const { ServiceDSC, Customer, Employee } = require('../config/db');
...
router.get('/', authenticate, async (req, res) => {
    try {
        const data = await ServiceDSC.findAll({ include, order: [['createdAt', 'DESC']] });
        res.json(data);
...
router.get('/expiring', authenticate, async (req, res) => {
    try {
        ...
        const data = await ServiceDSC.findAll({
            where: {
...
router.get('/:id', authenticate, async (req, res) => {
    try {
        const row = await ServiceDSC.findByPk(req.params.id, { include });
...
router.post('/', authenticate, async (req, res) => {
    try {
        const row = await ServiceDSC.create({ ...req.body, EmployeeId: req.user.id });
...
router.put('/:id', authenticate, async (req, res) => {
    try {
        const row = await ServiceDSC.findByPk(req.params.id);
        if (!row) return res.status(404).json({ message: 'Not found' });
...
router.put('/:id/followup', authenticate, async (req, res) => {
    try {
        ...
        await row.update({
            ...
            remarks: req.body.notes || row.remarks
        });
