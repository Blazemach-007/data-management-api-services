...
import { getCustomers, getAadhaar, createAadhaar, updateAadhaar, deleteService } from '@/services/serviceApi';

const empty = { CustomerId: '', aadhaar_number: '', update_type: 'Update', request_id: '', amount: '0', status: 'Pending', notes: '' };
...
export default function AadhaarPage() {
    ...
    const load = () => { setLoading(true); getAadhaar().then(setRecords).catch(console.error).finally(() => setLoading(false)); };
    useEffect(() => { load(); getCustomers().then(setCustomers).catch(console.error); }, []);

    const openAdd = () => { setEditing(null); setForm(empty); setShowForm(true); };
    const openEdit = (r: any) => { setEditing(r); setForm({ ...r, CustomerId: r.Customer?.id || r.CustomerId, amount: r.amount?.toString() || '' }); setShowForm(true); };
    ...
    const handleSubmit = async (e: any) => {
        ...
        try {
            ...
            if (editing) await updateAadhaar(editing.id, payload);
            else await createAadhaar(payload);
    ...
    const handleDelete = async (id: string) => {
        ...
        try { await deleteService('aadhaar', id); load(); } catch (err: any) { setMsg('❌ ' + err.message); }
    };
    ...
    const statusColor: any = { Completed: '#16a34a', Rejected: '#dc2626', Submitted: '#2563eb', Pending: '#d97706' };

    return (
        <div style={pg.wrap}>
            <div style={pg.header}>
                <div><h1 style={pg.title}>📋 Aadhaar Services</h1><p style={pg.sub}>{records.length} total records</p></div>
                <div style={{ display: 'flex', gap: 10 }}>
                    <input placeholder="Search..." value={search} onChange={e => setSearch(e.target.value)} style={pg.searchInput} />
                    <button onClick={openAdd} style={pg.addBtn}>+ Add Aadhaar</button>
                </div>
            </div>
            ...
            {showForm && (
                <div style={pg.modal}><div style={pg.modalBox}>
                    <h2 style={pg.modalTitle}>{editing ? 'Edit' : 'Add'} Aadhaar Service</h2>
                    <form onSubmit={handleSubmit} style={pg.form}>
                        ...
                        {[
                            ...
                            { name: 'request_id', label: 'Request ID / SRN' },
                            { name: 'amount', label: 'Amount (₹)', type: 'number' },
                        ].map(f => <div key={f.name}><label style={pg.label}>{f.label}</label><input name={f.name} value={form[f.name]} onChange={handleChange} type={f.type || 'text'} style={pg.input} /></div>)}
                        <label style={pg.label}>Update Type</label>
                        <select name="update_type" value={form.update_type} onChange={handleChange} style={pg.input}>
                            {['Update', 'New Enrolment', 'Correction', 'Reprint', 'Other'].map(s => <option key={s}>{s}</option>)}
                        </select>
                        <label style={pg.label}>Status</label>
                        <select name="status" value={form.status} onChange={handleChange} style={pg.input}>
                            {['Pending', 'Submitted', 'Completed', 'Rejected'].map(s => <option key={s}>{s}</option>)}
                        </select>
            ...
            {loading ? <p style={pg.loading}>Loading...</p> : (
                <div style={pg.tableWrap}><table style={pg.table}>
                    <thead><tr style={pg.thead}>{['Customer', 'Aadhaar No', 'Update Type', 'Request ID', 'Amount', 'Status', 'Date', 'Actions'].map(h => <th key={h} style={pg.th}>{h}</th>)}</tr></thead>
                    <tbody>{filtered.length === 0 ? <tr><td colSpan={8} style={pg.empty}>No records found</td></tr> : filtered.map(r => (
                        <tr key={r.id} style={pg.tr}>
                            ...
                            <td style={pg.td}>{r.aadhaar_number || '-'}</td>
                            <td style={pg.td}>{r.update_type}</td>
                            <td style={pg.td}>{r.request_id || '-'}</td>
                            <td style={pg.td}><strong>₹{r.amount?.toLocaleString()}</strong></td>
                            <td style={pg.td}><span style={{ ...pg.badge, color: statusColor[r.status] || '#64748b', backgroundColor: (statusColor[r.status] || '#64748b') + '20' }}>{r.status}</span></td>
                            <td style={pg.td}>{new Date(r.createdAt).toLocaleDateString('en-IN')}</td>
                            <td style={pg.td}>
