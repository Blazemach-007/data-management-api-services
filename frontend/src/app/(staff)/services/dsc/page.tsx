...
import { getCustomers, getDSC, createDSC, updateDSC, deleteService } from '@/services/serviceApi';

const empty = { CustomerId: '', cert_type: '', serial_number: '', issued_date: '', expiry_date: '', authority: '', amount: '', status: 'Active', follow_up_status: 'None', notes: '' };
...
export default function DSCPage() {
    ...
    const load = () => { setLoading(true); getDSC().then(setRecords).catch(console.error).finally(() => setLoading(false)); };
    ...
    const handleSubmit = async (e: any) => {
        ...
        try {
            ...
            if (editing) await updateDSC(editing.id, payload);
            else await createDSC(payload);
    ...
    const handleDelete = async (id: string) => {
        ...
        try { await deleteService('dsc', id); load(); } catch (err: any) { setMsg('❌ ' + err.message); }
    };
    ...
    const filtered = records.filter(r => {
        const matchSearch = (r.Customer?.full_name || '').toLowerCase().includes(search.toLowerCase()) || (r.serial_number || '').toLowerCase().includes(search.toLowerCase());
    ...
    return (
        <div style={pg.wrap}>
            <div style={pg.header}>
                <div><h1 style={pg.title}>🔐 DSC Management</h1><p style={pg.sub}>{records.length} total records</p></div>
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                    ...
                    <button onClick={openAdd} style={pg.addBtn}>+ Add DSC</button>
                </div>
            </div>
            ...
            {showForm && (
                <div style={pg.modal}><div style={pg.modalBox}>
                    <h2 style={pg.modalTitle}>{editing ? 'Edit' : 'Add'} DSC</h2>
                    <form onSubmit={handleSubmit} style={pg.form}>
                        ...
                        {[
                            { name: 'cert_type', label: 'Certificate Type' },
                            { name: 'serial_number', label: 'Serial Number' },
                            { name: 'issued_date', label: 'Issued Date', type: 'date' },
                            { name: 'expiry_date', label: 'Expiry Date', type: 'date' },
                            { name: 'authority', label: 'Certifying Authority' },
            ...
            {loading ? <p style={pg.loading}>Loading...</p> : (
                <div style={pg.tableWrap}><table style={pg.table}>
                    <thead><tr style={pg.thead}>{['Customer', 'Cert Type', 'Serial No', 'Issued', 'Expires', 'Amount', 'Status', 'Follow-up', 'Actions'].map(h => <th key={h} style={pg.th}>{h}</th>)}</tr></thead>
                    <tbody>{filtered.length === 0 ? <tr><td colSpan={9} style={pg.empty}>No records found</td></tr> : filtered.map(r => {
                        ...
                        return (
                            <tr key={r.id} style={pg.tr}>
                                ...
                                <td style={pg.td}>{r.cert_type || '-'}</td>
                                <td style={pg.td}>{r.serial_number || '-'}</td>
                                <td style={pg.td}>{r.issued_date || '-'}</td>
                                <td style={pg.td}>
