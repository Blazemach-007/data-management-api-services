...
import { getCustomers, getFastag, createFastag, updateFastag, deleteService } from '@/services/serviceApi';

const empty = { CustomerId: '', vehicle_number: '', vehicle_type: 'Car', bank: '', tag_id: '', chassis_no: '', amount: '', status: 'Completed', notes: '' };
...
export default function FastagPage() {
    ...
    const load = () => { setLoading(true); getFastag().then(setRecords).catch(console.error).finally(() => setLoading(false)); };
    ...
    const handleSubmit = async (e: any) => {
        ...
        try {
            ...
            if (editing) await updateFastag(editing.id, payload);
            else await createFastag(payload);
    ...
    const handleDelete = async (id: string) => {
        ...
        try { await deleteService('fastag', id); load(); } catch (err: any) { setMsg('❌ ' + err.message); }
    };
    ...
    const statusColor = (s: string) => s === 'Completed' ? '#16a34a' : s === 'Rejected' ? '#dc2626' : '#d97706';

    return (
        <div style={pg.wrap}>
            <div style={pg.header}>
                <div>
                    <h1 style={pg.title}>🚗 Fastag Management</h1>
                    <p style={pg.sub}>{records.length} total records</p>
                </div>
                <div style={{ display: 'flex', gap: 10 }}>
                    <input placeholder="Search..." value={search} onChange={e => setSearch(e.target.value)} style={pg.searchInput} />
                    <button onClick={openAdd} style={pg.addBtn}>+ Add Fastag</button>
                </div>
            </div>
            ...
            {showForm && (
                <div style={pg.modal}>
                    <div style={pg.modalBox}>
                        <h2 style={pg.modalTitle}>{editing ? 'Edit' : 'Add'} Fastag</h2>
                        <form onSubmit={handleSubmit} style={pg.form}>
                            ...
                            {[
                                ...
                                <div key={f.name}>
                                    ...
                                    <input name={f.name} value={form[f.name]} onChange={handleChange} required={f.required} type={f.type || 'text'} style={pg.input} />
                                </div>
                            ))}
            ...
            {loading ? <p style={pg.loading}>Loading...</p> : (
                <div style={pg.tableWrap}>
                    <table style={pg.table}>
                        <thead><tr style={pg.thead}>
                            {['Customer', 'Vehicle No', 'Type', 'Bank', 'Amount', 'Status', 'Date', 'Actions'].map(h => <th key={h} style={pg.th}>{h}</th>)}
                        </tr></thead>
                        <tbody>
                            {filtered.length === 0 ? (
                                <tr><td colSpan={8} style={pg.empty}>No records found</td></tr>
                            ) : filtered.map(r => (
                                <tr key={r.id} style={pg.tr}>
                                    ...
                                    <td style={pg.td}>{r.vehicle_number}</td>
                                    <td style={pg.td}>{r.vehicle_type}</td>
                                    <td style={pg.td}>{r.bank || '-'}</td>
...
const pg: any = {
    ...
    alert: { padding: '10px 16px', borderRadius: 8, marginBottom: 16, fontWeight: 600, fontSize: '0.9rem' },
    ...
    empty: { textAlign: 'center', color: '#94a3b8', padding: '30px' }
};
