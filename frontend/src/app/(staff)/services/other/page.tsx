...
import { getCustomers, getOther, createOther, updateOther, deleteService } from '@/services/serviceApi';

const empty = { CustomerId: '', service_name: '', description: '', amount: '', status: 'Completed', notes: '' };
...
export default function OtherServicesPage() {
    ...
    const load = () => { setLoading(true); getOther().then(setRecords).catch(console.error).finally(() => setLoading(false)); };
    ...
    const handleSubmit = async (e: any) => {
        ...
        try {
            ...
            if (editing) await updateOther(editing.id, payload);
    ...
    const handleDelete = async (id: string) => {
        ...
        try { await deleteService('other', id); load(); } catch (err: any) { setMsg('❌ ' + err.message); }
    };
    ...
    return (
        <div style={pg.wrap}>
            <div style={pg.header}>
                <div><h1 style={pg.title}>📦 Other Services</h1><p style={pg.sub}>{records.length} total records</p></div>
                <div style={{ display: 'flex', gap: 10 }}>
                    <input placeholder="Search..." value={search} onChange={e => setSearch(e.target.value)} style={pg.searchInput} />
                    <button onClick={openAdd} style={pg.addBtn}>+ Add Service</button>
                </div>
            </div>
            ...
            {showForm && (
                <div style={pg.modal}><div style={pg.modalBox}>
                    <h2 style={pg.modalTitle}>{editing ? 'Edit' : 'Add'} Other Service</h2>
                    <form onSubmit={handleSubmit} style={pg.form}>
                        ...
                        <select name="status" value={form.status} onChange={handleChange} style={pg.input}>
                            {['Pending', 'Completed'].map(s => <option key={s}>{s}</option>)}
                        </select>
            ...
            {loading ? <p style={pg.loading}>Loading...</p> : (
                <div style={pg.tableWrap}><table style={pg.table}>
                    <thead><tr style={pg.thead}>{['Customer', 'Service', 'Description', 'Amount', 'Status', 'Date', 'Actions'].map(h => <th key={h} style={pg.th}>{h}</th>)}</tr></thead>
                    <tbody>{filtered.length === 0 ? <tr><td colSpan={7} style={pg.empty}>No records found</td></tr> : filtered.map(r => (
                        <tr key={r.id} style={pg.tr}>
                            ...
                            <td style={pg.td}><strong>{r.service_name}</strong></td>
                            <td style={pg.td}>{r.description || '-'}</td>
                            <td style={pg.td}><strong>₹{r.amount?.toLocaleString()}</strong></td>
                            <td style={pg.td}><span style={{ ...pg.badge, backgroundColor: r.status === 'Completed' ? '#dcfce7' : '#fff7ed', color: r.status === 'Completed' ? '#16a34a' : '#d97706' }}>{r.status}</span></td>
