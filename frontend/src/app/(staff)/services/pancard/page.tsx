...
import { getCustomers, getPancard, createPancard, updatePancard, deleteService } from '@/services/serviceApi';

const empty = { CustomerId: '', applicant_name: '', pan_number: '', dob: '', father_name: '', form_type: '49A', amount: '', status: 'Pending', notes: '' };
...
export default function PancardPage() {
    ...
    const load = () => { setLoading(true); getPancard().then(setRecords).catch(console.error).finally(() => setLoading(false)); };
    ...
    const handleSubmit = async (e: any) => {
        ...
        try {
            ...
            if (editing) await updatePancard(editing.id, payload);
            else await createPancard(payload);
    ...
    const handleDelete = async (id: string) => {
        ...
        try { await deleteService('pancard', id); load(); } catch (err: any) { setMsg('❌ ' + err.message); }
    };

    const filtered = records.filter(r =>
        (r.applicant_name || r.Customer?.full_name || '').toLowerCase().includes(search.toLowerCase()) ||
        (r.pan_number || '').toLowerCase().includes(search.toLowerCase())
    );

    const statusColor = (s: string) => ({ Completed: '#16a34a', Rejected: '#dc2626', Submitted: '#2563eb', Pending: '#d97706' }[s] || '#64748b');

    return (
        <div style={pg.wrap}>
            <div style={pg.header}>
                <div><h1 style={pg.title}>🪪 PAN Card Management</h1><p style={pg.sub}>{records.length} total records</p></div>
                <div style={{ display: 'flex', gap: 10 }}>
                    <input placeholder="Search..." value={search} onChange={e => setSearch(e.target.value)} style={pg.searchInput} />
                    <button onClick={openAdd} style={pg.addBtn}>+ Add PAN Card</button>
                </div>
            </div>
            ...
            {showForm && (
                <div style={pg.modal}><div style={pg.modalBox}>
                    <h2 style={pg.modalTitle}>{editing ? 'Edit' : 'Add'} PAN Card</h2>
                    <form onSubmit={handleSubmit} style={pg.form}>
                        ...
                        <label style={pg.label}>Form Type</label>
                        <select name="form_type" value={form.form_type} onChange={handleChange} style={pg.input}>
                            {['49A', '49AA'].map(s => <option key={s}>{s}</option>)}
                        </select>
            ...
            {loading ? <p style={pg.loading}>Loading...</p> : (
                <div style={pg.tableWrap}><table style={pg.table}>
                    <thead><tr style={pg.thead}>{['Customer', 'Applicant', 'PAN No', 'Form', 'Amount', 'Status', 'Date', 'Actions'].map(h => <th key={h} style={pg.th}>{h}</th>)}</tr></thead>
                    <tbody>{filtered.length === 0 ? <tr><td colSpan={8} style={pg.empty}>No records found</td></tr> : filtered.map(r => (
                        <tr key={r.id} style={pg.tr}>
                            ...
                            <td style={pg.td}>{r.applicant_name}</td>
                            <td style={pg.td}>{r.pan_number || '-'}</td>
                            <td style={pg.td}>{r.form_type}</td>
                            <td style={pg.td}><strong>₹{r.amount?.toLocaleString()}</strong></td>
                            <td style={pg.td}><span style={{ ...pg.badge, color: statusColor(r.status), backgroundColor: statusColor(r.status) + '20' }}>{r.status}</span></td>
...
const pg: any = {
    ...
    searchInput: { padding: '8px 12px', border: '1px solid #cbd5e1', borderRadius: 8, fontSize: '0.9rem', width: 200 },
